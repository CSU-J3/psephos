...
- `tools/` vs `scripts/`: read-only offline analysis lives in `tools/` (`masterlist_corpus`, `sasts_dump`, `sasts_join`); anything that mutates a database lives in `scripts/` (`backfill_state_bills`, `backfill_supersession`) and is dry-run-by-default with an `--apply` gate.
