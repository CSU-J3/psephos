...
- `tools/` vs `scripts/`: read-only offline analysis lives in `tools/` (`masterlist_corpus`, `sasts_dump`, `sasts_join`, `lit_resolve_audit`, `status_audit`, `coverage_audit`); anything that mutates a database lives in `scripts/` (`backfill_state_bills`, `backfill_supersession`, `backfill_case_state`, `repair_latest_entry`, `rekey_slug_cases`, `backfill_appeal_metadata`) and is dry-run-by-default with an `--apply` gate. **Four of the six are one-time and already applied — do not schedule any of them.** `backfill_case_state` is superseded by `upsert_case` writing `state` every run, and `repair_latest_entry` by `write_entries` recomputing its column. `rekey_slug_cases` (2026-09-07) re-keyed the two unresolved slug `cases` rows onto their CourtListener docket ids, carrying their items; it has no work left and its guards would refuse anyway, the slug rows being gone. `backfill_appeal_metadata` (2026-09-07) filled `caption`/`filed_at`/`source_url` on those two rows — the columns `upsert_case` writes only on a fresh resolve, which the reuse path they now take never triggers; re-running it is harmless but spends two CourtListener requests to change nothing. `backfill_supersession` is the one that stays runnable: it gains a pair whenever a docket is continued elsewhere. `python -m tools.coverage_audit` is the standing coverage check: its **exit code is sections 1 and 4** — the reconciliation alarm (a row matching no seed and carrying no `superseded_by`) and the derived-column alarm (`cases.latest_entry_at` disagreeing with `MAX(case_entries.entry_at)`), both expect 0 — while sections 2 and 3 are reports that are expected non-empty: 7 unresolvable docket references and 1 terminated circuit row awaiting a cert petition. Do not read those two counts as failures. **Those figures are as-of, not invariants.** As of **2026-09-07** §1 reads **0** and the audit **exits 0** — Unit A landed, and that is the first 0 since 2026-08-20. (It read 6 on 09-06, unread for seventeen days, and 2 after Unit B; the residual MN and DC pair needed a re-key, the two `COURT_IDS` aliases and `canonicalCourt` at the read boundary before it could be asserted.) §2 reads **6**, not the 7 this line used to cite, because four of the seven references became held dockets and three new ones surfaced from their entries, which is the non-monotonicity the tool's own header describes. §3 still reads 1. Re-run it rather than quoting these; and read its exit status directly, never through a pipe to `head`, which reports the pager's status instead. See `docs/findings/reconciliation-alarm-2026-09-06.md`.
