...
**Standing check — bootstrap coverage. Now section 6 of `python -m tools.coverage_audit`, and it should be read there rather than pasted.** A docket with `entries_synced_at IS NULL` was never bootstrapped. This used to be a direct Turso `SELECT` a human was expected to remember, and unit C moved it into the audit for the reason the whole unit exists: the query was correct, available and unrun, exactly like the reconciliation alarm beside it. The daily `audit.yml` now runs it and opens a standing GitHub issue on a non-zero exit. The SQL stays below because the section is derived from it and a reader should be able to check one against the other; the correct form excludes superseded rows, since a terminated district docket that has been continued as a circuit appeal is complete, not unbootstrapped (handoff 13):
