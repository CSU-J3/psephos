...
### The 08-13 00:00Z run — one run that tests three things

**This is the only open item, and it is now three units riding on one run:** the 20h pin, the transport retry's first production exposure, and the seven-pair supersession reaching `data/cases.json`. The 08-12 day is read and closed below, and everything before it closed on the 2026-08-11 ~23:00Z sweep. Read those first if any of this is unfamiliar.

**Do not read before `2026-08-13T01:30Z`.** The 00:00Z run starts late and takes up to ~17 minutes, so an earlier read catches a partial log or no run at all and produces a false finding about the pin.

Handoff 33's four commits are **pushed** as of this update, so the next scheduled run is the first to execute the 20h gate, the transport retry, and the job bound — `timeout-minutes: 45`, raised from 30 by `9a98660` and pushed alongside this doc. Nothing ran between the push and the boundary: stamps landed 00:57:44–00:59:14Z on 08-12, plus the new 20h gate puts the boundary at **20:57Z on 08-12** — after that day's 18:00Z run and before 08-13 00:00Z. So the pin takes effect on the first run under it, with no partial day in between.

Read in one pass:

- **08-13 00:00Z — 24 due, 0 changed, 0 failed.** First run under the 20h gate. A number other than 24 means a row changed state in between, which is a finding, not noise; `SELECT COUNT(*) FROM cases WHERE status IS NULL OR status <> 'terminated'` is the check.
- **08-13 06:00, 12:00, 18:00Z — 0 due on each.**
- **08-14 00:00Z — 24 due again, same slot.** Two consecutive days on the same slot is the pin confirmed. Drift to a later slot is the pin failing, and the 20h arithmetic above is what it would falsify.
- **Any `hit a transport error` line, in any run.** Zero expected. Grep that exact substring: `db.py:116` prints `db: {what} hit a transport error ({exc}); retry {i} of ...`, and there is **no "Turso" in the string** — handoff 38 quoted it as `hit a Turso transport error`, which matches nothing and would come back clean whether or not the retry had fired. A false green in the shape of a successful check. A genuinely clean log is *not* evidence the retry works either, since it prints only on a catch, the same blindness `_log_429` has — but the first line ever logged is the evidence about whether these 502s are intermittent, and it is worth more than the retry itself.
- **Job wall clock against `timeout-minutes: 45`.** The bound is 45, not the 30 this line used to name; `9a98660` raised it and recorded the measured range, and `collect.yml:31` is the check. Expect wall clock inside **5m42s–16m42s**, the range measured across the eight scheduled runs of 08-11 and 08-12. A timeout kill would itself be the finding.
- **The next data commit's `data/cases.json` diff — exactly three case objects.** `72334676` → `73674243`, `72156765` → `73690636`, `71982149` → `73678095`, each `superseded_by` null to the target id, nothing else. Diff against `2a6a89b`. **A fourth changed object is drift and gets read before anything else.** Written before the commit exists so it can fail; applied to Turso on 08-12 and deliberately not exported locally, so the first run producing a non-empty diff carries it. Note the empty-diff path does not apply here: a run with no other changes still commits, because these three objects changed.
...
**Handoff 17 — supersession candidate generator. PLANNED, inputs now specified.** Note the renumbering: this was called 16 mid-session before the channel-count cache took that number. It used to carry a second half — handoff 14's dry-run display defect — which is **closed**: handoff 36 replaced the `PAIRS` echo with `describe()`, and handoff 37 exercised it on three new pairs. What remains is the generator, and it is next because its three inputs stopped being hypothetical: each was measured on real pairs during 36/37.

Hand-assertion is the bottleneck — three pairs became seven in a month, each found by a human noticing something.

**Placement and boundary.** `tools/supersession_candidates.py`, not `scripts/`. It is read-only offline analysis, which is the documented split in `CLAUDE.md`. **It proposes and never writes.** The mutation boundary stays exactly where it is: a human reads the report, pastes a pair into `PAIRS`, runs the dry-run, then `--apply`. The generator must not gain an `--apply` of its own, because its whole value is that a wrong candidate is cheap.

**Candidate set:** `SELECT case_id FROM cases WHERE status = 'terminated' AND superseded_by IS NULL`. Match and report on `case_id` only, never caption — two rows read `United States v. Oliver`.
...
Filter volumes, separately: **424 self-references**, **227 bracket entry-numbers** all excluded by requiring the year-dash form (`[6]`/`[32]` carry no dash), **14 unresolvable** occurrences over 9 distinct strings.
...
- **4th Cir. `73608654` ↔ `73690636`** — two real successors, mutually referencing through a related-case disclosure, both live. Killed by **P1** (neither terminated) and by **P2** (circuit↔circuit). Must stay dead under both.
- **W.D. Pa. `71453026` ↔ 6th Cir. `72347022`** — correctly appeal-shaped and wrong. Killed by **P1** (both terminated), and by **P3** if P1 is ever relaxed. This is the fixture that keeps the circuit map honest, since it is the only pair P3 catches alone.
- **The 1st Cir. three-way `73470017` / `73479340` / `73488435`** — one consolidation order producing a complete triangle from three rows. Killed by **P1** and **P2**. Pins the combinatorial shape: n rows named in one order yield n(n−1)/2 pairs, so a loosening here scales quadratically.

**Two limits to state up front.** It can only ever link rows the database already holds, so a successor that was never resolved is invisible to it — that is a coverage question for the tracker, and output 2 above is what surfaces it. And a docket-shaped string in an entry is not a link: require the proposed successor to *exist as a row* carrying that docket, not merely to be named in text, or an unrelated appeal number in a filing becomes a false pair.
