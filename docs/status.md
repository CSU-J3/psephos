...
### The 20h pin and the retry's first production exposure — the 08-13/08-14 runs

**This is the only open item.** The 08-12 day is read and closed below, and everything before it closed on the 2026-08-11 ~23:00Z sweep. Read those first if any of this is unfamiliar.

Handoff 33's four commits are **pushed** as of this update, so the next scheduled run is the first to execute the 20h gate, the transport retry, and `timeout-minutes: 30`. Nothing ran between the push and the boundary: stamps landed 00:57:44–00:59:14Z on 08-12, plus the new 20h gate puts the boundary at **20:57Z on 08-12** — after that day's 18:00Z run and before 08-13 00:00Z. So the pin takes effect on the first run under it, with no partial day in between.
...
- **Any `db: ... hit a transport error` line, in any run.** Zero expected. A clean log is *not* evidence the retry works — it prints only when it catches something, the same blindness `_log_429` has — but the first line ever logged is the evidence about whether these 502s are intermittent, and it is worth more than the retry itself.
- **Job wall clock against `timeout-minutes: 30`.** Runs measure 6–12 minutes, so a timeout kill would itself be the finding.
