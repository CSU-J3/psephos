...
**Input 3 — circuit-jurisdiction check (statutory, a veto and not a discovery channel).** Runs *after* successor kind is established from the successor's court type, never before. Appeal → assert the successor sits in the circuit that hears appeals from the predecessor's district. Refile → assert same state, and **do not run the circuit check at all**, or it fires backwards on M.D. Ga. → N.D. Ga.
...
**Two limits to state up front.** It can only ever link rows the database already holds, so a successor that was never resolved is invisible to it — that is a coverage question for the tracker, not something the generator can fix. And a docket-shaped string in an entry is not a link: require the proposed successor to *exist as a row* carrying that docket, not merely to be named in text, or an unrelated appeal number in a filing becomes a false pair. Michigan (`72347022`, 6th Cir. `26-1225`, terminated, no successor, no district row held) is the standing test case for the "surface as unresolvable rather than silently drop" path.
