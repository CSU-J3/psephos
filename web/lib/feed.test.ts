...
describe("ordering", () => {
  ...
  it("breaks a fetched_at tie by id descending, deterministically", () => {
    ...
    expect(buildFeed([a, b]).entries.map((e) => e.id)).toEqual([12, 11]);
    // Same input in the opposite order must produce the same output.
    expect(buildFeed([b, a]).entries.map((e) => e.id)).toEqual([12, 11]);
  });
  ...
  it("orders on fetched_at even when occurred_at disagrees", () => {
    ...
    expect(buildFeed([fresh, backdated]).entries.map((e) => e.id)).toEqual([1, 2]);
  });
...
describe("the cap, and the count it would otherwise hide", () => {
  it("reports the pre-cap total so the page can say what it dropped", () => {
    const rows = Array.from({ length: 63 }, (_, i) => entry({ id: i + 1 }));
    const feed = buildFeed(rows);
    expect(feed.entries).toHaveLength(FEED_LIMIT);
    expect(feed.total).toBe(63); // NOT 50 -- a silent top-N reads as "everything"
  ...
  it("is not truncated when the window fits", () => {
    ...
    expect(feed.entries).toHaveLength(29);
    expect(feed.total).toBe(29);
  ...
  it("handles an empty window", () => {
    expect(buildFeed([])).toEqual({ entries: [], total: 0, truncated: false });
  });
});
...
describe("grades", () => {
  it("keeps C3 items in the feed carrying their own grade", () => {
    ...
    expect(feed.entries.map((e) => `${e.admiralty_source}${e.admiralty_info}`).sort())
      .toEqual(["A1", "B2", "C3"]);
  });
});
